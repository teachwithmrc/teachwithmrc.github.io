// === digraphs_master_FINAL.js ===
// Combined: patterns/images + base digraph ladders + blend ladders + mode helper
...
// --- Auto-generated ladders that include ≥1 digraphWordsDetailed word (with blends) ---
...
// Build a subset where EVERY word contains at least one consonant blend.
const BLEND_CLUSTERS = [
...
function hasBlend(word){
  const w = String(word).toLowerCase();
  return BLEND_CLUSTERS.some(b => w.includes(b));
}

const digraphBlendOnlyLadders = (typeof digraphBlendLadders !== 'undefined')
  ? digraphBlendLadders.filter(path => path.every(hasBlend))
  : [];

/**
 * getLaddersByMode
 * "CVC"            -> digraphWordLadders
 * "Include Blends" -> digraphBlendLadders
 * "Only Blends"    -> digraphBlendOnlyLadders
 */
function getLaddersByMode(label){
  switch(label){
    case "Only Blends":
      return typeof digraphBlendOnlyLadders !== 'undefined' ? digraphBlendOnlyLadders : [];
    case "Include Blends":
...
// Expose globals for the HTML
if (typeof window !== 'undefined'){
  window.rWordPatterns = typeof rWordPatterns !== 'undefined' ? rWordPatterns : {};
  window.rWordImages = typeof rWordImages !== 'undefined' ? rWordImages : {};
